class UserFriendshipsController < ApplicationController
  ...
  def new
    if params[:friend_id]
      @friend = User.where(profile_name: params[:friend_id]).first  #profile_name is being received in friend_id in params; is this really a good thing?
  ...
  def index
    @user_friendship = UserFriendshipDecorator.decorate_collection friendship_association.load

    respond_with @user_friendship # if json, this will convert all the @user_friendship objects to json format
  end
  ...
  def block
    ...
    if @user_friendship.block!
      ...
      flash[:error] = "The friendship with #{@user_friendship.friend.full_name} could not be blocked."
    end
  end
